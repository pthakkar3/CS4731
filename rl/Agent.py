...
class Agent:
	...
	def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
		...
		if terminal:
			...
			self.v_table[lastState][action] = self.v_table[lastState][action] + self.learningRate * (reward + self.gamma*optimalAction - self.v_table[lastState][action])
	...
	def greedy(self, observation):
		...
		# YOUR CODE GOES BELOW HERE
		for index, action in enumerate(self.v_table[self.calculateFlatState(observation.worldState)]):
			if action == max(self.v_table[self.calculateFlatState(observation.worldState)]):
				return index
		# YOUR CODE GOES ABOVE HERE
		return 0
