...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		String chromo = "";
		//width is 205, but offsets add to 10 so want a chromosome that accounts for the whole level
		for (int i = 0; i < 195; i++) {
			chromo = chromo + stockDNA.charAt(rand.nextInt(stockDNA.length()));
		}
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		for (MyDNA individual : population) {
			if (individual.getFitness() > 0.8 || count >=2500) {
				decision = true;
			}
		}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		int toMutate = rand.nextInt(population.size()) + 1;
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		MyDNA picked = null;
		// YOUR CODE GOES BELOW HERE
		int toPick = rand.nextInt(population.size()) + 1;
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		for (int i = 0; i < this.getPopulationSize(); i++) {
			MyDNA fittest = getBestIndividual(allPopulation);
			finalPopulation.add(fittest);
			allPopulation.remove(fittest);
		}
