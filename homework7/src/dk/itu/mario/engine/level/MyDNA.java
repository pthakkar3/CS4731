...
public class MyDNA extends DNA
{
	...
	public MyDNA mutate ()
	{
		...
		String stockDNA = "abcdefghijklmnopqrstuvwxyz";
		int mutation = rand.nextInt(stockDNA.length());
		int mutationLocation = rand.nextInt(this.getChromosome().length());
		String newChromosome = "";
		for (int i = 0; i < this.getChromosome().length(); i++) {
			if (i == mutationLocation) {
				newChromosome = newChromosome + stockDNA.charAt(mutation);
			} else {
				newChromosome = newChromosome + this.getChromosome().charAt(i);
			}
		}
	...
	public ArrayList<MyDNA> crossover (MyDNA mate)
	{
		...
		int crossoverPoint = rand.nextInt(this.getChromosome().length());

		// Crossover as shown in class: abcd + wxyz = abyz + wxcd
		String chromo1 = this.getChromosome().substring(0, crossoverPoint) + mate.getChromosome().substring(crossoverPoint, mate.getChromosome().length());
		String chromo2 = mate.getChromosome().substring(0, crossoverPoint) + this.getChromosome().substring(crossoverPoint, this.getChromosome().length());

		offspring1.setChromosome(chromo1);
		offspring2.setChromosome(chromo2);
