...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        String chromo = dna.getChromosome();
		for (int x = 0; x < chromo.length(); x++) {
		    char gene = chromo.charAt(x);
		    if (gene == 's') {
		        this.setBlock(x + STARTOFFSET, DEFAULTHEIGHT - 3, COIN);
            } else if (gene == 'j') {
                this.setBlock(x + STARTOFFSET, DEFAULTHEIGHT - 6, ROCK);
                this.setBlock(x + STARTOFFSET, DEFAULTHEIGHT - 5, ROCK);
                this.setBlock(x + STARTOFFSET, DEFAULTHEIGHT - 4, ROCK);
		        this.setBlock(x + STARTOFFSET, DEFAULTHEIGHT - 3, ROCK);
            } else if (gene == 'k') {
		        this.setSpriteTemplate(x + STARTOFFSET, DEFAULTHEIGHT - 3, new SpriteTemplate(2, false));
            }
        }
