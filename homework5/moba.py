...
class MOBAAgent(VisionAgent):
	...
	def __init__(self, position, orientation, world, image = NPC, speed = SPEED, viewangle = 360, hitpoints = HITPOINTS, firerate = FIRERATE, bulletclass = MOBABullet):
		...
		self.safe = True # If false, agent takes damage from touching towers and bases
		self.safe_count = 0 # Count up until safe threshold
	...
	def collision(self, thing):
		...
		if isinstance(thing, Obstacle):
			self.die()
		# Agent takes damage from touching towers and bases if safe = False
		if not self.safe and (isinstance(thing, Tower) or isinstance(thing, Base)):
			self.damage(1)
	...
	def update(self, delta):
		VisionAgent.update(self, delta)
		# Count up until safe_count passes safethreshold, at which point the agent becomes unsafe
		self.safe_count = self.safe_count + 1
		if self.safe == True and self.safe_count > SAFETHRESHOLD:
			self.safe = False
...
class Base(Mover):
	...
	def spawnNPC(self, type, angle = 0.0):
		...
		if n < MAXSPAWN:
			...
			agent.start()
		print "Minion spawned: " + str(self.numSpawned) + " (team " + str(self.team) + ")"
		return agent
