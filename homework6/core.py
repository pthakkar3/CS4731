...
#		self.bulletspeed = bulletspeed
	...
	def update(self, delta):
		Mover.update(self, delta)
		self.drawHealth(self.world.background)
...
#			mag = reduce(lambda x, y: (x**2)+(y**2), direction)**0.5 
	...
	def drawHealth(self, surface):
		hit = self.hitpoints
		max = self.maxHitpoints
		w = self.getMaxRadius()

		colorVal = float(hit)/float(max)
		if colorVal > 1:
			colorVal = 1
		elif colorVal < 0:
			colorVal = 0
		...
		r = 255 - int(colorVal)
		g = int(colorVal)
		color = pygame.Color(r, g, 0, 255)
		loc = self.getLocation()
		surface.fill(color, pygame.Rect(loc[0]-w, loc[1] - w/2, l, 8))
