...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		# check if Sequence Node has any children, if not, then return True
		if self.getNumChildren() == 0:
			return True
		# this means that the Sequence has gotten through all the children and all have succeeded, so the node succeeds
		if self.getCurrentIndex() >= self.getNumChildren():
			return True
		childVal = self.getChild(self.getCurrentIndex()).execute(delta)
		# if child is true then we go to the next child
		if childVal is True:
			self.setCurrentIndex(self.getCurrentIndex() + 1)
			return None
		# if child is false we return false
		elif childVal is False:
			return False
		# this is in case the child takes more than one tick to complete execution
		else:
			return None
...
class Selector(BTNode):
	...
	def execute(self, delta = 0):
		...
		# check if Selector Node has any children, if not, then return False
		if self.getNumChildren() == 0:
			return False
		# this means that the Selector has gotten through all the children and all have failed, so the node fails
		if self.getCurrentIndex() >= self.getNumChildren():
			return False
		childVal = self.getChild(self.getCurrentIndex()).execute(delta)
		#if child is false then we go to the next child
		if childVal is False:
			self.setCurrentIndex(self.getCurrentIndex() + 1)
			return None
		#if child is true we return true
		elif childVal is True:
			return True
		#this is in case the child takes more than one tick to complete execution
		else:
			return None
