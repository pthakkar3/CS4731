...
class Hero(MOBAAgent):
	...
	def update(self, delta = 0):
		...
		if self.canareaeffect == False:
			...
			if self.areaEffectTimer >= self.areaEffectRate:
				self.canareaeffect = True
				self.areaEffectTimer = 0
		# Check if the agent is back in the clear (not touching base, tower, or edge of world)
		if self.safeTimer > 0:
			all_clear = True
			for base in self.world.getBases() + self.world.getTowers():
				if distance(self.getLocation(), base.getLocation()) <= self.getMaxRadius() + base.getMaxRadius():
					all_clear = False
					break
			if self.getLocation()[0] < 0 or self.getLocation()[1] < 0 or self.getLocation()[0] > self.world.dimensions[0] or self.getLocation()[1] > self.world.dimensions[1]:
				all_clear = False
			if all_clear:
				self.safeTimer = 0
	...
	def collision(self, thing):
		MOBAAgent.collision(self, thing)
		# If agent collides with base, tower, or edge of world for too long, start taking damage
		if isinstance(thing, Tower) or isinstance(thing, Base) or isinstance(thing, GameWorld):
			if self.safeTimer > SAFETHRESHOLD:
				self.die()
				print "die"
			else:
...
class MOBAWorld(GatedWorld):
	...
	def addBase(self, base):
		...
		if base.getTeam() not in self.score.keys():
			self.score[base.getTeam()] = 0
	...
	def drawWorld(self):
		...
		for team in self.score.keys():
			label = self.font.render((str(team) + ": " + str(self.score[team])), True, (0, 0, 0))
			self.screen.blit(label, (10, yPos))
			yPos += 25
		tickLabel = self.font.render("Ticks: " + str(self.ticks), True, (0, 0, 0))
		self.screen.blit(tickLabel, (10, yPos))
